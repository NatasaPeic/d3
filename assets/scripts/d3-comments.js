...
// Takes string and converts it into js date object
// This takes in a string that has a date as a year
let parseDate = d3.time.format("%Y").parse;



// Set the ranges
// define x scaling as well as range of the function
// time scale funcion for x axis data. it creates scaling function where range goes from 0 to the width of inner drawing space. we set domain later when we load the data
...
// Define the line
// define d3 path generate function
...
// Adds the svg canvas
// define svg container and inner drawing space
...
// up until now we have created code that we are going to use, now we need to call it

// Get the data
// building of chart happens inside of callback function
...
let chart1 = d3.csv("death-rate.csv", function(error, data) {
  ...
  // it applies anonymous function to each element of the array.
  ...
  //  we set domain later when we load the data
    ...
    svg.append("g")
        ...
        // it moves to the bottom of the inner drawing space
    ...
    dataNest.forEach(function(d) {
          // create line using d3 path generator
          // define path
         svg.append("path")
            .attr("class", "line")
